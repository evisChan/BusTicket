...
class Admin extends CI_Controller
{
	...
	public function daftar()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', 'swal("Succeed", "Berhasil Menambahkan Akun", "success");');
			redirect('backend/admin');
		}

	}
	public function viewadmin($tiket)
	{
		$data['title'] = "List Admin";
		$data['tiket'] = $this->db->query("SELECT * FROM tbl_admin WHERE kd_admin = '" . $tiket . "'")->row_array();
		if ($data['tiket']) {
			$this->load->view('backend/view_admin', $data);
		} else {
