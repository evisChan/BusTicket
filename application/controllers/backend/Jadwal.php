...
class Jadwal extends CI_Controller
{
	...
	public function tambahjadwal()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($asal == $tujuan['kd_tujuan']) {
				$this->session->set_flashdata('message', 'swal("Succeed", "Tujuan Jadwal Tidak Bisa Sama", "error");');
				...
				$this->session->set_flashdata('message', 'swal("Succeed", "Jadwal baru telah ditambahkan", "success");');
				redirect('backend/jadwal');
			}
	...
	public function viewjadwal($id = '')
	{
		...
		if ($sqlcek) {
			...
			$this->session->set_flashdata('message', 'swal("Failed", "Ada yang salah. Silakan Coba Lagi", "error");');
			redirect('backend/jadwal');
		}
	}
}
