...
class Profile extends CI_Controller
{
	...
	public function editprofile($id = '')
	{
		...
		$this->session->set_flashdata('message', 'swal("Success", "Data  Diubah", "success");');
	...
	public function changepassword($id = '')
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if (!password_verify($currentpassword, $pelanggan['password_pelanggan'])) {
				...
				$this->db->update('tbl_pelanggan', $update, $where);
				$this->session->set_flashdata('message', 'swal("Success", "Password Anda Berhasil Diganti", "success");');
				redirect('profile/profilesaya/' . $id);
			}
		}
